...
const PNMASK: [u8; 256] = [
	...
	8u8, 0x80u8, 0x40u8, 0x80u8, 0x20u8, 0x80u8, 0x40u8, 0x80u8, 0x10u8,
		0x80u8, 0x40u8, 0x80u8, 0x20u8, 0x80u8, 0x40u8, 0x80u8,
	4u8, 0x80u8, 0x40u8, 0x80u8, 0x20u8, 0x80u8, 0x40u8, 0x80u8, 0x10u8,
		0x80u8, 0x40u8, 0x80u8, 0x20u8, 0x80u8, 0x40u8, 0x80u8,
	8u8, 0x80u8, 0x40u8, 0x80u8, 0x20u8, 0x80u8, 0x40u8, 0x80u8, 0x10u8,
		0x80u8, 0x40u8, 0x80u8, 0x20u8, 0x80u8, 0x40u8, 0x80u8,
	2u8, 0x80u8, 0x40u8, 0x80u8, 0x20u8, 0x80u8, 0x40u8, 0x80u8, 0x10u8,
		0x80u8, 0x40u8, 0x80u8, 0x20u8, 0x80u8, 0x40u8, 0x80u8,
	8u8, 0x80u8, 0x40u8, 0x80u8, 0x20u8, 0x80u8, 0x40u8, 0x80u8, 0x10u8,
		0x80u8, 0x40u8, 0x80u8, 0x20u8, 0x80u8, 0x40u8, 0x80u8,
	4u8, 0x80u8, 0x40u8, 0x80u8, 0x20u8, 0x80u8, 0x40u8, 0x80u8, 0x10u8,
	...
	8u8, 0x80u8, 0x40u8, 0x80u8, 0x20u8, 0x80u8, 0x40u8, 0x80u8, 0x10u8,
		0x80u8, 0x40u8, 0x80u8, 0x20u8, 0x80u8, 0x40u8, 0x80u8,
	1u8, 0x80u8, 0x40u8, 0x80u8, 0x20u8, 0x80u8, 0x40u8, 0x80u8, 0x10u8,
		0x80u8, 0x40u8, 0x80u8, 0x20u8, 0x80u8, 0x40u8, 0x80u8,
	8u8, 0x80u8, 0x40u8, 0x80u8, 0x20u8, 0x80u8, 0x40u8, 0x80u8, 0x10u8,
		0x80u8, 0x40u8, 0x80u8, 0x20u8, 0x80u8, 0x40u8, 0x80u8,
	4u8, 0x80u8, 0x40u8, 0x80u8, 0x20u8, 0x80u8, 0x40u8, 0x80u8, 0x10u8,
...
impl Pink {
	...
	fn a(&mut self) -> i16 {
		...
		self.dec = self.dec & !0x800i32;
		self.lfsr = self.lfsr << 1i32;
		self.dec = self.dec | self.inc & 0x800i32;
		self.inc = self.inc ^ self.bit & 0x800i32;
		self.b()
	}

	fn b(&mut self) -> i16 {
		self.accu += self.inc - self.dec;
		self.lfsr = self.lfsr ^ self.bit & 0x46000001i32;
	...
	fn c(&mut self) -> i16 {
		self.bit = self.lfsr >> 31i32;
		self.dec = self.dec & !0x400i32;
		self.lfsr = self.lfsr << 1i32;
		self.dec = self.dec | self.inc & 0x400i32;
		self.inc = self.inc ^ self.bit & 0x400i32;
		self.b()
	}

	fn d(&mut self) -> i16 {
		self.bit = self.lfsr >> 31i32;
		self.dec = self.dec & !0x200i32;
		self.lfsr = self.lfsr << 1i32;
		self.dec = self.dec | self.inc & 0x200i32;
		self.inc = self.inc ^ self.bit & 0x200i32;
		self.b()
	}

	fn e(&mut self) -> i16 {
		self.bit = self.lfsr >> 31i32;
		self.dec = self.dec & !0x100i32;
		self.lfsr = self.lfsr << 1i32;
		self.dec = self.dec | self.inc & 0x100i32;
		self.inc = self.inc ^ self.bit & 0x100i32;
		self.b()
	}
	...
	fn f(&mut self, mask: i32) -> i16 {
		...
		self.lfsr = self.lfsr << 1i32;
		self.dec = self.dec | self.inc & mask;
		self.inc = self.inc ^ self.bit & mask;
		self.b()
	}
